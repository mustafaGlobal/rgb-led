...
void RGBLed::set_write( unsigned int* color , unsigned int bright ){
  ...
  unsigned int print_red = map( red_ , 0 , 255 , 0 , on_off_state_ ? 32 * bright_ - 1 : 0 );
  unsigned int print_green = map( green_ , 0 , 255 , 0 , on_off_state_  ? 32 * bright_ -1 : 0 );
  unsigned int print_blue = map( blue_ , 0 , 255 , 0 , on_off_state_  ? 32 * bright_ - 1 : 0 );
